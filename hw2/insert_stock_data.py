...
def fetch_stock_data(stock_code):
    """ 從 API 抓取指定股票的數據 """
    base_url = "https://mis.twse.com.tw/stock/api/getStockInfo.jsp?ex_ch=tse_{stock_code}.tw&json=1&delay=0"
    url = base_url.format(stock_code=stock_code)
...
def parse_stock_data(stock_data):
    ...
    return {
        ...
        "close_price": safe_float(stock_data.get("y", "0")),  # 收盤價
        "price_change": price_change,  # 漲跌價差
        "volume": safe_float(stock_data.get("v", "0")),  # 成交量
        "trade_value": 0,  # 成交金額 (API 無提供，手動設為 0)
        "trade_count": safe_float(stock_data.get("v", "0"))  # 成交筆數 (API 無提供，手動設為 0)
    }
...
def daily_crawler():
    ...
    for stock in stock_list:
        ...
        new_record = (
            ...
            parsed_data["low_price"], parsed_data["close_price"], parsed_data["price_change"],
